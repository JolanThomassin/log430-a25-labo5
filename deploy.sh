...
APP_DIR="$HOME/app-lab5"      # Dossier où ton app sera déployée
REPO_BRANCH="main"
ENV_FILE=".env"
...
if [ -d ".git" ]; then
    ...
    echo "Cloning repo..."
    git clone -b "$REPO_BRANCH" https://github.com/JolanThomassin/log430-a25-labo5 .
fi
...
mysql -h 127.0.0.1 -uroot -p"$DB_ROOT_PASSWORD" labo05_db < db-init/init.sql
